...
biblioteca reproducen exactamente las ecuaciones que el grupo ya
validó en su Proyecto 2 (lab_celdas_solares_1.py): Beer-Lambert,
probabilidad de colección fc(x), EQE/IQE, diodo implícito con Rs/Rp,
malla frontal de dedos de plata y dependencia con temperatura.
...
def reparto_zona_deplecion(W_dep, NA, ND):
    """Reparte el ancho total de deplección entre el lado n y el lado p
    por neutralidad de carga: NA·x_p = ND·x_n (mismo número de cargas
    fijas descubiertas a cada lado). Como NA << ND (base p ligeramente
    dopada, emisor n fuertemente dopado), la deplección se extiende
    casi por completo hacia la BASE (x_p >> x_n): el lado más dopado
    necesita menos ancho para exponer la misma carga.

    Retorna (x_n, x_p) en las mismas unidades que W_dep."""
    x_n = W_dep * NA / (NA + ND)
    x_p = W_dep * ND / (NA + ND)
    return x_n, x_p
...
def calcular_EQE_IQE_preciso(wavelength_nm, alpha_cm, R, phi0,
    ...
    IQE = EQE / (1.0 - R)
    return np.clip(EQE, 0.0, 1.0 - R), np.clip(IQE, 0.0, 1.0)
